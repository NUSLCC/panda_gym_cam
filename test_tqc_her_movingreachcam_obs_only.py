...
if __name__=="__main__":
    ...
    env_id = "PandaMovingReachCamJointsDense-v3"
    num_cpu = 16
    ...
    save_path="./logs/",
    name_prefix=datetime.now().strftime('philip4_tqc_deep_movingreach_obsonly_joints_%H_%M_%d'),
    ...
    tmp_path = "./tmp/"+datetime.now().strftime('tqc_dual_philip4_movingreach_obsonly_joints_%H_%M_%d')
    ...
    # model = TQC.load("logs/philip4_tqc_deep_movingreach_obsonly_joints_850000_steps", env = env)
    # model.load_replay_buffer("logs/philip4_tqc_deep_movingreach_obsonly_joints_replay_buffer_850000_steps", truncate_last_traj=False)
    # print(f'Replay buffer size is {model.replay_buffer.size()}')
    # tmp_path = "./tmp/"+"tqc_dual_philip4_movingreach_joints_22_38_29"
    # new_logger = configure(tmp_path, ["stdout", "csv", "tensorboard"])
    # model.set_logger(new_logger)
