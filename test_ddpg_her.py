...
if __name__=="__main__":
    ...
    save_path="./logs/",
    name_prefix="philip4_ddpg_deep_pick_and_place",
    save_replay_buffer=True,
    save_vecnormalize=True,
    )
    
    # Training from scratch:
    model = DDPG(policy="MultiInputPolicy",env=env, batch_size=1024, gamma=0.95, learning_rate=1e-4, verbose=1, 
   ...
   # print(model.policy)
    tmp_path = "./tmp/"+datetime.now().strftime('ddpg_deep_dual_philip4_pickandplace_%H_%M_%d')
    ...
    # model = SAC.load("logs/philip4_ddpg_deep_pick_and_place_1200000_steps", env = env)
    # model.load_replay_buffer("logs/philip4_ddpg_deep_pick_and_place_replay_buffer_1200000_steps", truncate_last_traj=False)
    # print(f'Replay buffer size is {model.replay_buffer.size()}')
    # tmp_path = "./tmp/"+"ddpg_deep_dual_philip4_pickandplace_07_50_27"
    # new_logger = configure(tmp_path, ["stdout", "csv", "tensorboard"])
    # model.set_logger(new_logger)
    # model.learn(total_timesteps=1_500_000, callback=checkpoint_callback, reset_num_timesteps=False, progress_bar=True)
    
    model.save("ddpg_her_philip4_pickandplace")
