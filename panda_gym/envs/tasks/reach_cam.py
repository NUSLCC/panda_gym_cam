...
class ReachCam(Task):
    ...
    def _create_scene(self) -> None:
        ...
        self.object_initial_velocity = np.random.uniform(
            np.array(self.object_velocity_max) / 2, self.object_velocity_max
        )
    ...
    def render_from_stationary_cam(
        ...
        """
        cam_pos = self.sim.get_link_position(
            "stationary_camera", self.stationary_cam_link
        )
        cam_orn = np.array(
            p.getQuaternionFromEuler(
                [math.radians(90 - self.stationary_cam_pitch_angle), 0, math.pi / 2]
            )
        )
        cam_pos[0] = cam_pos[0] - 0.013 * math.cos(
            math.radians(90 - self.stationary_cam_pitch_angle)
        )  # 13 mm is half of L515 cam thickness, but need to use trigonometry because the camera is rotated 45 deg
        cam_pos[2] = cam_pos[2] - 0.013 * math.sin(
            math.radians(90 - self.stationary_cam_pitch_angle)
        )
        rot_matrix = np.array(
            ...
            3, 3
        )  # 3x3 rotation matrix (right, forward, up by columns)
        ...
        view_matrix = self.sim.physics_client.computeViewMatrix(
            cam_pos, target_position, up_vec
        )
        ...
        proj_matrix = self.sim.physics_client.computeProjectionMatrixFOV(
            fov, aspect_ratio, nearVal, farVal
        )
        rgb_img = self.sim.physics_client.getCameraImage(
            cam_width,
            cam_height,
            view_matrix,
            proj_matrix,
    ...
    def reset(self) -> None:
        (
            self.robot_cam_initial_x,
            self.robot_cam_initial_y,
            self.robot_cam_initial_z,
        ) = self.sim.get_link_position("panda_camera", self.cam_link)
    ...
    def _sample_goal(self) -> np.ndarray:
        ...
        )  # z offset for the sphere center
    ...
    def is_terminated(
        ...
        d = distance(achieved_goal, desired_goal)
        return np.array(
            d < self.distance_threshold or d > self.far_distance_threshold, dtype=bool
        )
