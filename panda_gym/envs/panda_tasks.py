...
from panda_gym.envs.tasks.pick_and_place_cam import PickAndPlaceCam
...
from panda_gym.envs.tasks.moving_reach_cam import MovingReachCam
...
from panda_gym.envs.tasks.lift_cam import LiftCam
...
class PandaLiftCamEnv(RobotCamTaskEnv):
    """Lift task wih Panda Cam robot.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = PandaCam(sim, block_gripper=False, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = LiftCam(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position)
        super().__init__(
            robot,
            task,
            render_width=render_width,
...
class PandaPushCamEnv(RobotCamTaskEnv):
    ...
    def __init__(
        ...
        task = PushCam(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position)
        super().__init__(
            robot,
            task,
            render_width=render_width,
...
class PandaReachCamObstacleEnv(RobotCamTaskEnv):
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = PandaCam(sim, block_gripper=True, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = ReachCamObstacle(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position)
        super().__init__(
            robot,
            task,
            render_width=render_width,
...
class PandaMovingReachCamEnv(RobotCamTaskEnv):
    """Reach task wih Panda Cam robot.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = PandaCam(sim, block_gripper=True, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = MovingReachCam(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position)
        super().__init__(
            robot,
            task,
            render_width=render_width,
