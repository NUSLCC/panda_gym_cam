...
from panda_gym.envs.tasks.slide_cam import SlideCam
...
class PandaSlideCamEnv(RobotCamTaskEnv):
    """Slide task wih Panda robot.
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = PandaCam(sim, block_gripper=True, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = SlideCam(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position)
        super().__init__(
            robot,
            task,
            render_width=render_width,
