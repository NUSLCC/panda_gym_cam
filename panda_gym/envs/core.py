...
class RobotCamTaskEnv(gym.Env):
    ...
    def _get_obs(
        ...
        # robot_obs = self.robot.get_obs().astype(np.uint8)  # robot cam
        task_obs = self.task.get_obs().astype(np.uint8)  # static cam
        ...
        desired_joint_angles = self.robot.inverse_kinematics(
            ...
            position=desired_goal_coords, 
            orientation=np.array([1.0, 0.0, 0.0, 0.0]))[:7].astype(np.float32) # remove fingers angles
