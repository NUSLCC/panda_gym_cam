...
if __name__=="__main__":
    env_id = "PandaReachCamJointsDense-v3"
    ...
    # print(model.policy)
    tmp_path = "./tmp/"+datetime.now().strftime('sac_rgb_sine_moving_deformcnn_j_kine_%H_%M_%d')
    # set up logger
    new_logger = configure(tmp_path, ["stdout", "csv", "tensorboard"])
    model.set_logger(new_logger)
    model.learn(total_timesteps=700_000, progress_bar=True)
    model.save("sac_rgb_sine_moving_deformcnn_j_kine")
