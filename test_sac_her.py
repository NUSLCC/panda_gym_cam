...
from panda_gym.utils import CombinedExtractor
...
if __name__=="__main__":
    ...
    model = SAC(policy="MultiInputPolicy",env=env, batch_size=512, gamma=0.95, learning_rate=1e-4, verbose=1, 
                ...
                policy_kwargs=dict(
                    ...
                    features_extractor_kwargs=dict(vit_output_dim = 256, device_id = 1),
    ...
    tmp_path = "./tmp/"+datetime.now().strftime('sac_dual_philip4_tinyViT_%H_%M_%d')
    # set up logger
    new_logger = configure(tmp_path, ["stdout", "csv", "tensorboard"])
    model.set_logger(new_logger)

    model.learn(total_timesteps=700_000, progress_bar=True)
    model.save("sac_her_philip4_tinyViT")
