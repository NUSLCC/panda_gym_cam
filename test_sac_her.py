...
from panda_gym.utils import NatureCNN
...
if __name__=="__main__":
    ...
    model = SAC(policy="CnnPolicy",env=env, batch_size=256, gamma=0.95, learning_rate=1e-4, verbose=1, 
    ...
    tmp_path = "./tmp/"+datetime.now().strftime('sac_dual_cnn_noher_%H_%M_%d')
    # set up logger
    new_logger = configure(tmp_path, ["stdout", "csv", "tensorboard"])
    model.set_logger(new_logger)

    model.learn(total_timesteps=700_000, progress_bar=True)
    model.save("sac_her_panda_cnn_noher")