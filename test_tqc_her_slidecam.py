...
if __name__=="__main__":
    ...
    save_path="./logs/",
    name_prefix=datetime.now().strftime('philip4_tqc_deep_slide_%H_%M_%d'),
    ...
    tmp_path = "./tmp/"+datetime.now().strftime('tqc_dual_philip4_slide_%H_%M_%d')
    ...
    # model = TQC.load("logs/philip4_tqc_deep_slide_100000_steps", env = env)
    # model.load_replay_buffer("logs/philip4_tqc_deep_slide_replay_buffer_100000_steps", truncate_last_traj=False)
    # print(f'Replay buffer size is {model.replay_buffer.size()}')
    # tmp_path = "./tmp/"+"tqc_dual_philip4_slide_23_08_12"
    # new_logger = configure(tmp_path, ["stdout", "csv", "tensorboard"])
    # model.set_logger(new_logger)
