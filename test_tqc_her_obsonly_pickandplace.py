...
if __name__=="__main__":
    ...
    save_path="./logs/",
    name_prefix=datetime.now().strftime('philip4_tqc_deep_obsonly_pickandplace_%H_%M_%d'),
    ...
    model = TQC(policy="MultiInputPolicy",env=env, batch_size=256, gamma=0.95, learning_rate=1e-4, verbose=1, 
                ...
                buffer_size=20000, replay_buffer_class=None, device="cuda:1",
                # Parameters for HER
    ...
    tmp_path = "./tmp/"+datetime.now().strftime('tqc_dual_philip4_obsonly_pickandplace_%H_%M_%d')
    ...
    # model = TQC.load("logs/philip4_tqc_deep_obsonly_pickandplace_1710000_steps", env = env)
    # model.load_replay_buffer("logs/philip4_tqc_deep_obsonly_pickandplace_replay_buffer_1710000_steps", truncate_last_traj=False)
